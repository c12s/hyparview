...
func NewHyParView(config Config, self data.Node, connManager transport.ConnManager, logger *log.Logger) (*HyParView, error) {
	h := &HyParView{
		...
		joinCh:            make(chan JoinCmd, 10),
		leaveCh:           make(chan LeaveCmd, 10),
		msgCh:             make(chan MsgEvent, 10),
		connDownCh:        make(chan ConnDownEvent, 10),
		shuffleCh:         make(chan ShuffleEvent, 10),
		stopShuffleCh:     make(chan ShuffleEvent, 10),
		peerUpCh:          make(chan PeerUpEvent, 10),
		peerDownCh:        make(chan PeerDownEvent, 10),
	}
